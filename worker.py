...
#        self.optimizer.zero_grad()
    ...
    def steal_task_from_worker(self):
        # Implement work-stealing logic here
        # For example, randomly choose a victim worker and steal a task
        victim = random.choice(all_workers)
        if not victim.task_queue.empty():
            stolen_task = victim.task_queue.get()
            self.task_queue.put(stolen_task)

    def run(self):
        while True:
            if self.idle and not self.task_queue.empty():
                self.idle = False
                task = self.task_queue.get()
                self.process_task(task)
                self.idle = True
            else:
                # Check if the worker's queue is empty and steal a task if needed
                if self.task_queue.empty():
                    self.steal_task_from_worker()
                # Continue normal execution
                victim = random.choice(all_workers)
                task = self.steal_task(victim)
                if task:
                    self.process_task(task)